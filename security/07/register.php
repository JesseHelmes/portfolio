...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if($uploadOk){
		register($_POST["username"], $_POST["password"], $target_file, $_POST["email"]);
	}
}


function register($username, $password, $target_file, $email){
	//hash password
	...
	if(password_verify($password, $hash)){
		...
		//Insert resultaat
		$sql = $conn->prepare("INSERT INTO users (username, password, profielfoto, email) VALUES (?, ?, ?, ?)");

		$sql->bind_param("ssss", $username, $hash, $target_file, $email);
